...
public class DashFunAsrFileTranscriberActivity extends Activity implements INativeFileTransCallback, View.OnClickListener, OnItemSelectedListener {
    private static final String TAG = "FunAsrFT";
    ...
    private final int MAX_TASKS = 1; // 这里同时只跑一个任务。每个任务可以同时识别多个文件。
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(DashFunAsrFileTranscriberActivity.this,
    ...
    private void getModelList() {
        ...
        model.add("fun-asr");
        model.add("fun-asr-2025-08-25");
        model.add("fun-asr-mtl");
        model.add("fun-asr-mtl-2025-08-25");
        ArrayAdapter<String> spinnerModel = new ArrayAdapter<String>(
                DashFunAsrFileTranscriberActivity.this,
        ...
        try {
            ...
            if (file_urls_array.size() > 0) {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(DashFunAsrFileTranscriberActivity.this, "请输入音频链接",
    ...
    private boolean checkNotInitToast() {
        if (!mInit) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(DashFunAsrFileTranscriberActivity.this, "SDK未成功初始化.", Toast.LENGTH_LONG).show();
                }
            });
            return false;
        } else {
        ...
        if (event == Constants.NuiEvent.EVENT_FILE_TRANS_CONNECTED) {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    Toast.makeText(DashFunAsrFileTranscriberActivity.this, msg_show,
                            Toast.LENGTH_SHORT).show();
                }
            });
        }
