...
// 本样例展示在线一句话语音识别/翻译使用方法
// Android SDK 详细说明：
public class DashGummySpeechRecognizerActivity extends Activity implements INativeNuiCallback, AdapterView.OnItemSelectedListener {
    private static final String TAG = "GummySR";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(DashGummySpeechRecognizerActivity.this, version_text, Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void getSrcLanguageList() {
        ...
        ArrayAdapter<String> spinnerLans = new ArrayAdapter<String>(
                DashGummySpeechRecognizerActivity.this,
    ...
    private void doInit() {
        ...
        if (ret == Constants.NuiResultCode.SUCCESS) {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(DashGummySpeechRecognizerActivity.this,
...
//            nls_config.put("extend_config", extend_config);
    ...
    private String genInitParams(String debugpath) {
        ...
        try{
            ...
            // 若此处设置长效apikey, 则不需要每次startDialog()都设置
            // 注意！不推荐在这里设置apikey。推荐每次在startDialog()时设置临时鉴权token。
//            object.put("apikey", g_apikey);
    ...
    private void removeWordsSafely(JSONObject root) {
        if (root == null) return;
    ...
    public void onNuiEventCallback(Constants.NuiEvent event, final int resultCode,
        ...
        if (event == Constants.NuiEvent.EVENT_ASR_STARTED) {
            ...
            showText(resultView, "EVENT_ASR_STARTED");
            ...
            JSONObject showJsonObj = JSON.parseObject(asrResult.asrResult);
            removeWordsSafely(showJsonObj);
            String showResponse = showJsonObj.toJSONString();
            appendText(resultView, "EVENT_ASR_RESULT " + showResponse);
            ...
            removeWordsSafely(showJsonObj);
            String showResponse = showJsonObj.toJSONString();
            appendText(resultView, "EVENT_ASR_PARTIAL_RESULT " + showResponse);
        } else if (event == Constants.NuiEvent.EVENT_ASR_ERROR) {
            // asrResult在EVENT_ASR_ERROR中为错误信息，搭配错误码resultCode和其中的task_id更易排查问题，请用户进行记录保存。
            appendText(resultView, asrResult.asrResult);
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(DashGummySpeechRecognizerActivity.this, "ERROR with " + resultCode,
            ...
            appendText(resultView, "EVENT_ASR_ERROR " + asrResult.allResponse);
        } else if (event == Constants.NuiEvent.EVENT_VAD_START) {
            appendText(resultView, "EVENT_VAD_START");
            ...
            appendText(resultView, "EVENT_MIC_ERROR");
